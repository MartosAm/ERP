...
var TOKEN_KEY = "erp_tkn";
var USER_KEY = "erp_usr";
var MARGEN_EXPIRACION_S = 60;
var TokenService = class _TokenService {
  ...
  estaExpirado() {
    ...
    if (!payload?.exp)
      return true;
    const ahoraEnSegundos = Math.floor(Date.now() / 1e3);
...
var AuthService = class _AuthService {
  constructor() {
    ...
    this._usuario = signal(this.tokenService.getUsuarioGuardado());
    this.usuario = this._usuario.asReadonly();
    this.estaAutenticado = computed(() => !!this._usuario() && !this.tokenService.estaExpirado());
    this.esAdmin = computed(() => this._usuario()?.rol === "ADMIN");
    this.esCajero = computed(() => this._usuario()?.rol === "CAJERO");
  }
  /** Login: guarda token en memoria + sessionStorage vía TokenService */
  login(credenciales) {
    return this.api.post("auth/login", credenciales).pipe(tap((res) => {
  ...
  validarSesion() {
    ...
    return this.api.get("auth/perfil").pipe(tap((usuario) => {
...
@angular/router/fesm2022/router.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-QCMV2OHP.js.map
